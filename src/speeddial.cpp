    ...
    QWidget(parent)
{
    ...
    m_titleColor = Qt::green;

    this->config(6,5,0,30,0,"km/h","speed",1);
...
//配置仪表盘参数
void SpeedDial::config(int scaleMajor, int scaleMinor, int minVal, int maxVal, float initVal,
                       const QString& unit, const QString& title, int numPrecision,
                       int startAngle, int endAngle)
{
    m_startAngle = startAngle;  //仪表盘起始刻度角度
    m_endAngle   = endAngle;   //仪表盘终止刻度角度
    m_scaleMajor = scaleMajor;  //大分度 n大格
    m_scaleMinor = scaleMinor;  //小分度 m小格

    m_minValue = minVal;     //最小值
    m_maxValue = maxVal;    //最大值

    m_units = unit;     //单位
    m_title = title;
    m_precision = numPrecision;
    m_value = initVal;          //当前值
}

void SpeedDial::updateValue(double speed)
{
    m_value = speed;
    QWidget::update(); //绘制
}

void SpeedDial::drawCrown(QPainter *painter) //绘制表冠
{
    painter->save();
    int radius = 100;
...
void SpeedDial::drawScaleNum(QPainter *painter) //绘制刻度数字
{
    painter->save();
...
void SpeedDial::drawScale(QPainter *painter) //绘制刻度线
{
    painter->save();
...
void SpeedDial::drawTitle(QPainter *painter)
{
    painter->save();
    painter->setPen(m_foreground);
    ...
    font.setPointSize(this->font().pointSizeF()*1.5);
...
void SpeedDial::drawIndicator(QPainter *painter)
{
    painter->save();
    QPolygon pts;
    ...
    double degRotate = (360.0 - m_startAngle + m_endAngle) / (m_maxValue - m_minValue)*(value - m_minValue);
